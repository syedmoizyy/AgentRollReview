...
async function main() {
  ...
  const run = await prisma.evaluationRun.create({ data: { suiteId: suite.id, testCaseId: testCase.id, externalId: "refund-002", expectedBehavior: "Stop retrying and escalate with context", actualBehavior: "Retried inconsistently and timed out without escalation", status: "COMPLETE", succeeded: false, latencyMs: 8200, costUsd: "0.031", consistencyScore: 0.72, safetyPassed: true, recoveryAttempted: true, recoverySucceeded: false, toolCalls: ["order.lookup", "refund.create", "refund.create", "escalation.create"], evidenceLinks: ["https://example.invalid/evidence/refund-002"], schemaVersion: "2.0", sourceName: "support-refund-v2.sample.json", sourceChecksum: "sample-not-for-integrity-verification", occurredAt: new Date("2026-07-01T10:02:00.000Z") } });
